...
export type ServiceCategory = 'Construction' | 'Structures' | 'MEP' | 'Telecom & IT' | 'General Trading & Manpower';
...
const telecomSeoEnhancements: Record<string, Pick<Service, 'primaryKeyword' | 'secondaryKeywords'> & Required<Pick<Service, 'directAnswer' | 'componentsSummary' | 'mainComponents' | 'reassurance'>>> = {
  'wired-network': {
    ...
    componentsSummary: 'A wired LAN may use copper for shorter endpoint connections and fibre for longer backbones or electrically isolated links. Indoor containment differs from underground fibre cable installation through ducts, handholes or manholes. Direct-buried network cable needs an approved route, suitable protection and defined civil reinstatement. Managed switches, cabinets, terminations, labels and agreed tests complete the connection. This wired infrastructure can also form part of a wider Extra-Low Voltage (ELV) scope where the project links structured cabling, network and security systems under one coordinated design.',
  ...
  'pbx-telephone-exchange': {
    ...
    componentsSummary: 'An office phone system can combine an IP-PBX or compatible telephone exchange, SIP or carrier gateways, IP desk phones, switches, cabling, extensions, hunt groups, call routing and IVR. A VoIP telephone system still depends on network quality, power, security and provider interfaces. PBX migration should preserve agreed numbers and essential call flows through a planned cutover and test schedule. Voice infrastructure can be coordinated as part of a wider Extra-Low Voltage (ELV) scope alongside structured cabling and network systems where the project links these disciplines together.',
  ...
  'door-access-security': {
    ...
    componentsSummary: 'A card or biometric access-control door typically combines a reader, controller, credential database, suitable lock, door contact, request-to-exit device, emergency release interfaces where required, power backup and structured cabling. Magnetic lock installation must suit the door and approved egress design. Time-attendance integration depends on compatible hardware, software, policies and confirmed data responsibilities. Access-control infrastructure can also be coordinated as part of a wider Extra-Low Voltage (ELV) scope alongside CCTV, structured cabling and network systems where the project requires it.',
  ...
  'public-address-systems': {
    ...
    componentsSummary: 'Commercial audio systems may use ceiling, wall or horn speakers selected for the space, supported by amplifiers with suitable power and headroom, microphones, source equipment, mixers or controllers, zone selection and dedicated cabling. A school announcement or warehouse paging system needs intelligible speech under real noise conditions. Emergency or life-safety use is excluded unless separately designed and evidenced. Public address infrastructure can be coordinated as part of a wider Extra-Low Voltage (ELV) scope alongside structured cabling and other building systems where the project links these disciplines together.',
...
export const categories = ['All', 'Construction', 'Structures', 'MEP', 'Telecom & IT', 'General Trading & Manpower'] as const;
